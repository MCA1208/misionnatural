...
function page() {
  ...
  const handleRowUpdate = async (newRow, oldRow) => {
    ...
    var checkDateStart = new Date(startDateFormat);
    var checkDateEnd = new Date(endDateFormat);

    //#region
    if (checkDateStart == "Invalid Date" || checkDateEnd == "Invalid Date") {
      Swal.fire({
        title: "Error en el formato de la fecha!",
        text: "El formato correcto es dd/mm/aaaa",
        icon: "error",
        confirmButtonText: "Cerrar",
        timer: 3000,
      });
    ...
    var getHourAndMinStart = startHourFormat.split(":");
    var getHourAndMinEnd = endHourFormat.split(":");

    if (
      getHourAndMinStart.length > 1 ||
      getHourAndMinStart[0] > 24 ||
      getHourAndMinStart[1] > 60 ||
      getHourAndMinEnd.length > 1 ||
      getHourAndMinEnd[0] > 24 ||
      getHourAndMinEnd[1] > 60
    ) {
      Swal.fire({
        title: "Error en el formato de la hora!",
        text: "El formato correcto es hh:mm",
        icon: "error",
        confirmButtonText: "Cerrar",
        timer: 3000,
      });
